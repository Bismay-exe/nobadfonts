...
import { AlignLeft, AlignCenter, AlignRight, Type, LayoutTemplate, Settings2, BoxSelect } from 'lucide-react';
...
export default function FontTester({ font }: FontTesterProps) {
    ...
    return (
        <div className="bg-white rounded-4xl border-2 border-black overflow-hidden flex flex-col h-full shadow-lg">
            {/* Top Navigation Bar */}
            <div className="flex border-b border-gray-100 bg-gray-50/50">
                ...
                <button
                    ...
                    `}
                >
                    <LayoutTemplate size={18} /> Real-World Context
                </button>
            </div>
            ...
            {viewMode === 'type' ? (
                <>
                    ...
                    <div className="border-b border-gray-100 bg-white p-2">
                        <div className="flex gap-2 mb-2 p-2 bg-gray-50 rounded-xl overflow-x-auto no-scrollbar">
                            <button
                                onClick={() => setActiveTab('settings')}
                                className={`px-4 py-2 rounded-lg text-xs font-bold flex items-center gap-2 transition-colors whitespace-nowrap
                                    ${activeTab === 'settings' ? 'bg-black text-white' : 'bg-white text-gray-600 hover:bg-gray-200'}
                                `}
                            >
                            ...
                            <button
                                ...
                                className={`px-4 py-2 rounded-lg text-xs font-bold flex items-center gap-2 transition-colors whitespace-nowrap
                                    ${activeTab === 'features' ? 'bg-black text-white' : 'bg-white text-gray-600 hover:bg-gray-200'}
                                `}
                            >
                            ...
                            {/* Preset Buttons */}
                            <button onClick={() => setText(TEXT_PRESETS.sentence)} className="px-3 py-1.5 bg-white border border-gray-200 rounded-md text-xs font-medium hover:border-black transition-colors">Abc</button>
                            <button onClick={() => setText(TEXT_PRESETS.alphabet)} className="px-3 py-1.5 bg-white border border-gray-200 rounded-md text-xs font-medium hover:border-black transition-colors">A-Z</button>
                            <button onClick={() => setText(TEXT_PRESETS.numerals)} className="px-3 py-1.5 bg-white border border-gray-200 rounded-md text-xs font-medium hover:border-black transition-colors">123</button>
                            <button onClick={() => setText(TEXT_PRESETS.paragraph)} className="px-3 py-1.5 bg-white border border-gray-200 rounded-md text-xs font-medium hover:border-black transition-colors">¶</button>
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 animate-in fade-in slide-in-from-top-2 duration-200">
                                    ...
                                    <div className="flex items-end gap-2">
                                        <div className="flex bg-gray-100 rounded-lg p-1 gap-1 flex-1">
                                            <button onClick={() => setTextAlign('left')} className={`flex-1 p-1 rounded ${textAlign === 'left' ? 'bg-white shadow-sm' : 'text-gray-400'}`}><AlignLeft size={16} className="mx-auto" /></button>
                                            <button onClick={() => setTextAlign('center')} className={`flex-1 p-1 rounded ${textAlign === 'center' ? 'bg-white shadow-sm' : 'text-gray-400'}`}><AlignCenter size={16} className="mx-auto" /></button>
                                            <button onClick={() => setTextAlign('right')} className={`flex-1 p-1 rounded ${textAlign === 'right' ? 'bg-white shadow-sm' : 'text-gray-400'}`}><AlignRight size={16} className="mx-auto" /></button>
                                        </div>
                    ...
                    {/* Preview Area */}
                    <div
                        className="flex-1 relative overflow-auto transition-colors duration-300 min-h-100"
                    ...
                    >
                        ...
                        <div className="absolute bottom-4 right-4 pointer-events-none opacity-50">
                            <p className="text-[10px] font-mono font-bold uppercase tracking-widest text-current rotate-90 origin-bottom-right translate-y-full" style={{ color: textColor }}>
                                NO BAD FONTS TESTER
                            </p>
                        </div>
                    </div>
